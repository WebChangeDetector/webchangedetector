...
class WebChangeDetector_Admin_AJAX {
	...
	public function ajax_post_url() {
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Missing -- Getting nonce value for verification
	...
	public function ajax_get_batch_comparisons_view() {
		...
		if ( $this->admin && $this->admin->dashboard_handler && method_exists( $this->admin->dashboard_handler, 'load_comparisons_view' ) ) {
			...
			// phpcs:disable WordPress.Security.NonceVerification.Missing -- Nonce was verified above
	...
	public function ajax_update_sync_types_with_local_labels() {
		...
		try {
				...
				if ( empty( $default_sync_types ) ) {
					$default_sync_types = array(
						...
						array(
							...
							'post_type_name' => get_taxonomy( 'category' )->labels->name, // category because we use the slug here, not rest_base.
						),
					);
				}

				$website_details['sync_url_types'] = $default_sync_types;
				\WebChangeDetector\WebChangeDetector_Admin_Utils::log_error( 'WCD AJAX: Created default sync types: ' . wp_json_encode( $default_sync_types ), 'update_sync_types_with_local_labels', 'debug' );
			}
			...
			\WebChangeDetector\WebChangeDetector_Admin_Utils::log_error( 'WCD AJAX: Update result: ' . wp_json_encode( $update_result ), 'update_sync_types_with_local_labels', 'debug' );
