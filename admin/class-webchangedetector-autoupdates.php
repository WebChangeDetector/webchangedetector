...
class WebChangeDetector_Autoupdates {
	...
	// Only for debugging. Overwrites the last_checked of wp to be able to trigger manually the wp_version_check cron.
	 ...
	 */
	 ...
	 public function wcd_cron_check_post_queues() {
		 ...
		 if(count($response['data']) === 0) {
			 $comparisons = $this->wcd->get_comparisons_v2(['batch' => $post_sc_option['batch_id']]);
			 $mail_body = '<style>
								table {
	...
	public function auto_update($update, $item, $type) {
		...
		if(current_time("H:i") < $auto_update_settings['auto_update_checks_from'] ||
			...
			error_log('Canceling auto updates: ' . current_time('H:i') .
