...
class WebChangeDetector_Screenshot_Action_Handler {
	 ...
	 *
	 * @param array $data The action data including URLs and settings.
	 * @return array Result with success status and message.
	 */
	public function handle_bulk_screenshots( $data ) {
		try {
			$urls = $data['urls'] ?? array();
			$sc_type = sanitize_text_field( $data['sc_type'] ?? 'manual' );
			$device = sanitize_text_field( $data['device'] ?? 'desktop' );
			
			if ( empty( $urls ) ) {
				return array(
					'success' => false,
					'message' => 'No URLs provided for screenshots.',
				);
			}

			$group_uuid = $this->admin->manual_group_uuid;
			$successful_requests = 0;
			$errors = array();

			foreach ( $urls as $url ) {
				$result = \WebChangeDetector\WebChangeDetector_API_V2::take_screenshot_single_url_v2(
					$group_uuid,
					$url,
					$sc_type,
					$device
				...
				if ( $result['success'] ?? false ) {
					...
					$errors[] = 'Failed for ' . $url . ': ' . ( $result['message'] ?? 'Unknown error' );
				}
			}

			$message = sprintf( 
				'%d of %d screenshots initiated successfully.', 
				$successful_requests, 
				count( $urls ) 
			);
			...
			return array(
				'success' => false,
				'message' => 'Error processing bulk screenshots: ' . $e->getMessage(),
			);
		}
	}
